...
Ids         = [0.4,     'foto99e83cda40fd2d3cd0a4d11485dffca2', '/sys/class/thermal/thermal_zone0/temp',    '/sys/bus/w1/devices/$ID1/temp',   '/sys/bus/w1/devices/$ID2/temp]
...
def getTime(mode, value=None):
    ...
    if mode == 0:
        ...
        if strftime('%H:%M:%S') == '00:00:00':
            ...
            return False
    elif mode == 9 and value != None and value != '':
...
def tick(timeOld=None):
    timeNew = getTime(1)
    if timeOld != timeNew:
        root.clock.config(text=timeNew)
        timeOld = timeNew
        refresh()
    root.clock.after(200, tick)
def refresh():
    if getTime(5) == True:
        errorMsg(2, False, 1, 'functions > tick', 'Refreshing Temperatures')
...
joke = Label(
    root,
    font=('Helvetica', 30),
    text='Hier könnte deine Werbung stehen! :P',
    fg='white',
    bg='black',
)
joke.place(
